...
function sourceLabel(type){
  return {youtube:"YouTube",direct:"Lien direct",local:"Fichier iPhone"}[type]??"Vidéo";
}
...
function openPlayer(item){
  ...
  playerEpisode.textContent=`Épisode ${item.episode} · ${statusLabel(item.status)} · ${sourceLabel(item.sourceType)}`;
